...
class UpdateRequest extends FormRequest
{
    ...
    public function rules()
    {
        return [
            ...
            'dni'  => 'required|max:8|min:8|unique:users,dni,'.$this->route('vendedor')->id,
            'email'  => 'required|email|unique:users,email,'.$this->route('vendedor')->id,
            // 'estado' => 'string',
            'telefono'  => 'required|unique:users,telefono,'.$this->route('vendedor')->id,
